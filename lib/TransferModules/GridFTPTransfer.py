...
class GridFTPTransfer(TransferBase):
    ...
    def __init__(self, config): 
        self.config = config
        self.cmd = self.config.get("gridftp.cmd");
    ...
    def setupPushCmd(self):
        ...
        pushcmd = gftp + " " + self.config.get("data_stream.directory") + "/" + f + " gsiftp://" + \
        self.config.get("outgoing.target_host") + ":" + str(self.config.get("gridftp.port")) + \
        "//" +  self.config.get("outgoing.target_dir") + "/" + f
        if self.config.get("outgoing.target_uses_arrival_monitor"):
            pushcmd += "; " + gftp + " " + self.config.get("data_stream.directory") + "/" + ctl_file_name + \
            " gsiftp://" + self.config.get("outgoing.target_host") + ":" + str(self.config.get("gridftp.port")) + \
    ...
    def setupPushThanksCmd(self):
        ...
        self.thankyou_file_path = thankyou_file_path
        thankyoucmd = gftp + " " + self.config.get("data_stream.directory") + "/" + thankyou_file_name + " gsiftp://" + \
        self.config.get("outgoing.target_host") + ":" + str(self.config.get("gridftp.port")) + \
        "//" +  self.config.get("outgoing.target_dir") + "/" + thankyou_file_name
        return thankyoucmd
    ...
    def setupStopFileCmd(self):
        ...
        self.stopname = name
        self.stoptf = tf
        pullstop = self.cmd + " gsiftp://" + self.config.get("outgoing.target_host") + \
        ":" + str(self.config.get("gridftp.port")) + "//" + self.config.get("outgoing.target_dir") + \
    ...
    def setupTransfer(self, f):
        ...
        if str(grv.code) == "Failure":
            ...
            os.fsync(tf)
            setupCredential = "myproxy-logon -S -s " + self.config.get("gridftp.proxy") + " -l " \
            + self.config.get("gridftp.username") + " < " + name
